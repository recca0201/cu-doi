...
const SESSION_DIR = path.resolve(process.env.MTV_AIDLC_CHROME_SESSION_DIR || DEFAULT_SESSION_DIR);
const SESSION_FILE = path.join(SESSION_DIR, 'browser-session.json');
const AUTH_SESSION_FILE = path.join(SESSION_DIR, 'auth-session.json');
...
function ensureSessionDir() {
  ...
  try {
    ...
    log('Failed to tighten session directory permissions:', e.message);
  }
}
...
function writeJsonFileSecure(filePath, data) {
  ...
  try {
    ...
    log('Failed to tighten session file permissions:', e.message);
  }
}
...
function writeSession(wsEndpoint) {
  try {
    writeJsonFileSecure(SESSION_FILE, {
      wsEndpoint,
      timestamp: Date.now()
    });
  } catch (e) {
...
export function saveAuthSession(authData) {
  try {
    ...
    writeJsonFileSecure(AUTH_SESSION_FILE, merged);
