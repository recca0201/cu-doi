#!/usr/bin/env python3
"""CLI helpers for AI-DLC units decomposition artifacts."""
...
DECOMP_SUFFIX = "_units_decomposition.md"


@dataclass
class DecompPaths:
    """Resolved units-decomposition artifact paths."""
...
def next_artifact_id(requirements_root: Path) -> str:
    """Determine the next 3-digit sequential ID from existing decomposition files."""
    if not requirements_root.exists():
        return "001"

    pattern = re.compile(r"^(\d{3})_.*_units_decomposition\.md$")
    used_ids = set()
    for path in requirements_root.iterdir():
...
def get_template_path() -> Path:
    """Resolve the bundled units decomposition template."""
    return Path(__file__).resolve().parents[1] / "assets" / "units_decomposition_template.md"
...
def render_decomp_frontmatter(
    feature_slug: str,
    artifact_id: str,
    created_date: str,
    repo_root: Optional[Path] = None,
) -> str:
    """Render metadata frontmatter for a units-decomposition artifact."""
...
def render_decomp_template(
    feature_slug: str,
    artifact_id: str,
    created_date: str,
    repo_root: Optional[Path] = None,
) -> str:
    """Render the units decomposition file from the bundled template."""
    template = get_template_path().read_text()
    body = template.replace("[Project Name]", slug_to_display_name(feature_slug))
    return (
        render_decomp_frontmatter(feature_slug, artifact_id, created_date, repo_root)
        + body.rstrip()
        + "\n"
    )
...
def resolve_paths(
    ...
    """Resolve paths for a units-decomposition artifact."""
    ...
    requirements_root = resolved_docs_root / REQUIREMENTS_DIR_NAME

    resolved_id = artifact_id or next_artifact_id(requirements_root)
    filename = f"{resolved_id}_{feature_slug}{DECOMP_SUFFIX}"

    return DecompPaths(
        repo_root=resolved_repo_root,
        docs_root=resolved_docs_root,
        requirements_root=requirements_root,
        decomp_file=requirements_root / filename,
...
def init_decomp(
    ...
    repo_root: Optional[Path] = None,
    docs_root: Optional[Path] = None,
) -> tuple[DecompPaths, bool]:
    """Create a units-decomposition scaffold if the file does not already exist."""
    paths = resolve_paths(feature_slug, artifact_id, repo_root, docs_root)
    if paths.decomp_file.exists():
        return paths, False

    paths.requirements_root.mkdir(parents=True, exist_ok=True)
    today = date.today().isoformat()
    paths.decomp_file.write_text(
        render_decomp_template(feature_slug, paths.artifact_id, today, paths.repo_root)
    )
    return paths, True
...
def get_status(
    ...
    """Return status information for a units-decomposition artifact."""
    paths = resolve_paths(feature_slug, artifact_id, repo_root, docs_root)
    exists = paths.decomp_file.exists()
    status = "missing"
    if exists:
    ...
    return {
        ...
        "decomp_path": str(paths.decomp_file),
...
def build_parser() -> argparse.ArgumentParser:
    """Build the units-decomp CLI parser."""
    parser = argparse.ArgumentParser(description="AI-DLC units decomposition CLI")
    parser.add_argument(
        "command",
        choices=["init", "status"],
    ...
    parser.add_argument(
        "--id",
        dest="artifact_id",
        help="3-digit artifact ID (e.g. 003); auto-incremented when omitted",
    )
    parser.add_argument(
        "--repo-root",
        type=Path,
...
def main() -> None:
    """Run the units-decomp CLI."""
    parser = build_parser()
    args = parser.parse_args()

    try:
        if args.command == "init":
            paths, created = init_decomp(
                args.feature_slug,
            ...
            if created:
                print(f"Created units decomposition: {paths.decomp_file}")
            else:
                print(f"Units decomposition already exists: {paths.decomp_file}")
            return
        ...
        if args.command == "status":
            ...
            print(f"ID: {status['artifact_id']}")
            print(f"Path: {status['decomp_path']}")
