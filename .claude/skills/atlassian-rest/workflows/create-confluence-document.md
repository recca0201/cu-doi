...
> ⚠️ **Agent file writing rule:** Do NOT use shell heredoc, `printf`, or `node -e "..."` to write temp files — these corrupt content in AI agent terminal environments. Instead, use the `create_file` tool to write `/tmp/confluence-body.html`, then reference it with `--body-file`. See the "How to Write Temp Files" section in SKILL.md for the full rule (applies to all Jira and Confluence operations).

**Step 1 — use the `create_file` tool (NOT a shell command):**
```
File path: /tmp/confluence-body.html
Content:   <your storage format HTML or markdown>
```
...
> ⚠️ Write `/tmp/body-with-images.html` using the `create_file` tool — not shell heredoc. See SKILL.md "How to Write Temp Files".
