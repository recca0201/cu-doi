...
## ⚠️ ADF Paragraph Behavior — Read Before Writing Description Files

Jira descriptions are rendered via ADF (Atlassian Document Format). Marklassian converts your markdown to ADF following **CommonMark rules**:

- A **blank line** (`\n\n`) = new paragraph = visual line break in Jira ✅
- A **single newline** (`\n`) between lines = soft break = lines collapse into one paragraph ❌

**Bad — all fields collapse into one run-on line in Jira:**
```markdown
**Type:** Spike
**Milestone:** MVP1 - M1 - Stage 1 B/L and BDR Readiness Dashboard
**Sprint:** Mock Sprint 1 - Core Search and Display
```

**Good — use a markdown table for key-value metadata:**
```markdown
| Field | Value |
|-------|-------|
| Type | Spike |
...
**Also good — blank lines between each field:**
```markdown
**Type:** Spike
...
This applies to any inline key-value metadata block. When writing description files from source documents (e.g. mock sprint stories), always convert `**Key:** Value` sequences into either a table or blank-separated paragraphs.
