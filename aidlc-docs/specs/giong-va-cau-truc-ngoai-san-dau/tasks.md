...
- [x] 6.1 Viết test cho năm trạng thái `locked`, `unlocked`, `current`,
  `completed`, `skipped`; mỗi node có hit target ≥48dp và một semantic label đầy đủ.
- [x] 6.2 Dựng node tròn/bo tròn với số ở giữa, 0–3 sao, lock icon, gold
  outline/glow cho current và badge/icon chữ cho skipped. Không dùng màu đơn độc.
- [x] 6.3 Giữ extent node ổn định giữa các state để grid không nhảy.

### 7. Section chapter và grid 4 cột

- [x] 7.1 Viết test: 4 header, mỗi header có `earned/max`; mỗi chapter có đúng
  5 node theo `levelId`; chapter sau không lấp ô trống của chapter trước.
- [x] 7.2 Dùng một `CustomScrollView`/sliver tree; mỗi section có header và
  `SliverGrid` 4 cột. Không dùng scroll lồng, `ChapterTrail` hoặc `TrailPainter`.
- [x] 7.3 Nhóm dự phòng cho arena ngoài `kChapters` vẫn hiện sau chương 4.

### 8. Tự định vị theo section/grid

- [x] 8.1 Viết test offset cho màn 1, 8 và 20; offset gồm extent header, grid
  row và khoảng section thật, được clamp và áp trước khung đầu.
- [x] 8.2 `targetArenaId` hợp lệ thắng luật suy ra; giá trị lạ rơi về
  `targetLevelId`; người mới/đã xong mở đầu danh sách.
- [x] 8.3 Không gọi `animateTo` khi mở; sau khi người chơi cuộn tay, locale/text
  scale đổi không tự đưa họ về đích.

### 9. Visual QA màn chọn màn

- [x] 9.1 Golden 390 × 844 phải khớp hierarchy của hình tham chiếu nhưng dùng
  đúng 4 chapter, không gắn cả 20 màn vào “Chương 1”.
- [x] 9.2 Kiểm current/locked/completed/skipped, tên chapter VI/EN, tổng sao,
  snackbar màn khoá và route tới màn mở.
- [x] 9.3 Chạy `flutter test` và `flutter analyze`; kiểm mắt golden trước khi kết luận.
...
- [x] 15.4 Run test to verify it passes
  ...
  - Command: `flutter test test/ui/game_screen_dialogue_test.dart`
  - Expected: PASS.
- [x] 15.5 Chốt overlay kết quả không overflow
...
- [x] 17.1 Chốt ranh giới không bị phá
...
- [x] 17.2 Chạy toàn bộ test và phân tích tĩnh
