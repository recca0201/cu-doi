...
//
// Outputs: lib/sim/arenas.dart, tools/solver/campaign.json, and the mirrored
// kArenas block in tools/solver/sim.js.
...
// Keep this at least as dense as verify.js. A 361-angle sweep missed narrow
// one-shot windows in arenas 2 and 5 that the 721-angle verifier found.
const ANGLE_STEPS = 721, SPREAD = 85;
const ANGLES = (() => {
  const out = [];
...
const campaignData = CAMPAIGN.map((a) => ({
...
function dartArena(a) {
  const lines = [
    '  ArenaSpec(',
    `    id: ${a.id},`,
    `    name: ${dartString(a.name)},`,
    `    nameEn: ${dartString(a.nameEn)},`,
    `    hint: ${dartString(a.hint)},`,
    `    hintEn: ${dartString(a.hintEn)},`,
    `    shots: ${a.shots},`,
    '    targets: <TargetSpec>[',
    ...a.targets.map((x) => `      TargetSpec(V2(${x.pos.x}, ${x.pos.y}), ${x.requiredBanks}, palette: ${x.palette}),`),
    '    ],',
  ];
  if (a.blocks.length) {
    lines.push('    blocks: <BlockSpec>[',
      ...a.blocks.map((x) => `      BlockSpec(${x.left}, ${x.top}, ${x.right}, ${x.bottom}),`),
      '    ],');
  }
  if (a.deflectors.length) {
    lines.push('    deflectors: <DeflectorSpec>[',
      ...a.deflectors.map((x) => `      DeflectorSpec(V2(${x.a.x}, ${x.a.y}), V2(${x.b.x}, ${x.b.y})),`),
      '    ],');
  }
  lines.push(`    starThresholds: <int>[${a.starThresholds.join(', ')}],`, '  ),');
  return lines.join('\n');
}

function jsArena(a) {
  const targets = a.targets.map((x) => `target(${x.pos.x}, ${x.pos.y}, ${x.requiredBanks}, ${x.palette})`).join(', ');
  const blocks = a.blocks.map((x) => `block(${x.left}, ${x.top}, ${x.right}, ${x.bottom})`).join(', ');
  const deflectors = a.deflectors.map((x) => `{ a: new V2(${x.a.x}, ${x.a.y}), b: new V2(${x.b.x}, ${x.b.y}) }`).join(', ');
  return [
    '  {',
    `    id: ${a.id}, name: ${JSON.stringify(a.name)}, shots: ${a.shots},`,
    `    hint: ${JSON.stringify(a.hint)},`,
    `    targets: [${targets}],`,
    `    blocks: [${blocks}],`,
    `    deflectors: [${deflectors}],`,
    `    starThresholds: [${a.starThresholds.join(', ')}],`,
    '  },',
  ].join('\n');
}

const dartOutput = `import 'arena.dart';
import 'geometry.dart';

/// The generated 20-arena campaign.
///
/// Geometry is authored in tools/solver/campaign.js. Required banks, shot
/// budgets, and star thresholds are tuned from the real simulation over
/// ${ANGLE_STEPS} aim angles per board state. Do not edit this file by hand.
const List<ArenaSpec> kArenas = <ArenaSpec>[
${CAMPAIGN.map(dartArena).join('\n')}
];
...
const jsonPath = path.resolve(__dirname, 'campaign.json');
const dartPath = path.resolve(__dirname, '../../lib/sim/arenas.dart');
const simPath = path.resolve(__dirname, 'sim.js');
fs.writeFileSync(jsonPath, JSON.stringify(campaignData, null, 2) + '\n', 'utf8');
fs.writeFileSync(dartPath, dartOutput, 'utf8');

const simSource = fs.readFileSync(simPath, 'utf8');
const generatedJs = `const kArenas = [\n${CAMPAIGN.map(jsArena).join('\n')}\n];`;
const arenaBlockPattern = /const kArenas = \[[\s\S]*?\n\];\r?\n\r?\nmodule\.exports/;
if (!arenaBlockPattern.test(simSource)) {
  throw new Error('Could not locate the generated kArenas block in sim.js');
}
const nextSimSource = simSource.replace(
  arenaBlockPattern,
  `${generatedJs}\n\nmodule.exports`,
);
fs.writeFileSync(simPath, nextSimSource, 'utf8');
console.log(`wrote ${path.relative(process.cwd(), dartPath)}, campaign.json, and synced sim.js`);
