...
void main() {
  testWidgets('third loss offers skip and confirms before spending', (
    WidgetTester tester,
  ) async {
    ...
    await tester.tap(find.byKey(const Key('skip-arena-button')));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 500));
    expect(find.text('Bỏ qua màn này?'), findsOneWidget);
    expect(repo.value.coins, 500);
    await tester.tap(find.text('Dùng xu và bỏ qua'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 500));
    expect(repo.value.coins, 350);
    expect(repo.value.isSkipped(1), isTrue);
  });
}
